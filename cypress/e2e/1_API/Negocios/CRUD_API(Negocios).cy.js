...
describe('CRUD API (/Deals)', function () {
    ...
    it('Editar Contato', function () {
      cy.readFile('cypress/fixtures/Negocio(API)/POST(Negocio).json')
        .then(registro => {      
      cy.request({
        method: 'PATCH',
        url: baseUrl + '/Deals(' + registro.value[0].Id + ")",
        headers: { 
            'User-Key' : `${apiKey}` 
        },
        ...payload
        ...
        expect(response.status).to.eq(200)
        cy.writeFile('cypress/fixtures/Negocio(API)/PATCH(Negocio).json', response.body)
            })
        })
    })
